    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float node_2289 = 0.0;
                float node_4436 = 0.0;
                float transform = saturate((((node_5613 + ( (o.uv0.g - _saikai) * (1.0 - node_5613) ) / ((_saikai+0.1) - _saikai))+(node_2289 + ( (o.uv2.g - _saikai) * (1.0 - node_2289) ) / ((_saikai+0.07) - _saikai)))*(node_4436 + ( (o.uv2.r - _shuipingjuhe) * (1.0 - node_4436) ) / ((_shuipingjuhe+0.07) - _shuipingjuhe))*(-1.0)));
...
////// Emissive:
                ...
                float4 node_6000 = _Time + _TimeEditor;
                float2 node_3278 = (i.uv1+node_6000.g*float2(0,-0.01));
                float4 _faxianliudong_var = tex2D(_faxianliudong,TRANSFORM_TEX(node_3278, _faxianliudong));
                float node_5613 = 0.0;
                float node_2289 = 0.0;
                float node_4436 = 0.0;
                float transform = saturate((((node_5613 + ( (i.uv0.g - _saikai) * (1.0 - node_5613) ) / ((_saikai+0.1) - _saikai))+(node_2289 + ( (i.uv2.g - _saikai) * (1.0 - node_2289) ) / ((_saikai+0.07) - _saikai)))*(node_4436 + ( (i.uv2.r - _shuipingjuhe) * (1.0 - node_4436) ) / ((_shuipingjuhe+0.07) - _shuipingjuhe))*(-1.0)));
                float3 emissive = (((_tietu1_var.rgb*_coler1.rgb)+(_kuangjia.rgb*_miaobian_var.rgb)+(((1.0 - saturate((sceneZ-partZ)/_bianyuanfaguang))+pow((1.0-max(0,dot(normalDirection, viewDirection))),exp(_saikai))+_faxianliudong_var.rgb)*_kuangjia.rgb))*(1.0 - transform));
