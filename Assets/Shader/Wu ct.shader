...
Shader "Shader Forge/wu ct" {
    Properties {
        ...
        _WUDAXIAO ("WU DA XIAO", Range(0, 3)) = 1.467792
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 node_7777 = _Time + _TimeEditor;
                float2 node_1103 = (i.uv0+node_7777.g*float2(0.1,0));
...
/////// Diffuse:
                ...
                float3 diffuseColor = ((_darkcoler.rgb+(step((1.0 - attenuation),_bianyuan)*_lightstrong*_lightcoler.rgb*(_norqiangdu+step(_norbianyuan,max(0,dot(i.bitangentDir,viewReflectDirection))))))*_LightColor0.rgb*lerp(_wucoler.rgb,_bacemap_var.rgb,node_7896));
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 node_1103 = (i.uv0+node_9310.g*float2(0.1,0));
                float4 _wu_var = tex2D(_wu,TRANSFORM_TEX(node_1103, _wu));
                float node_7896 = saturate(((lerp( node_1947, (1.0 - node_1947), _ZHENFAN )*exp(_WUDAXIAO))-_wu_var.r));
                float3 normalLocal = lerp(float3(0.1,0.1,0.1),_Normals_var.rgb,node_7896);
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
